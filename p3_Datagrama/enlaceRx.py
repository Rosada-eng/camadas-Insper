...
class RX(object):
    ...
    def getBuffer(self, nData):
        ...
        self.threadPause()
        b           = self.buffer[self.fisica.HeaderLen : self.fisica.HeaderLen + nData ]
        if self.buffer[self.fisica.HeaderLen + nData : self.fisica.HeaderLen + nData + 6] == END_ORDER: #@ C2 -- end order
            self.buffer = self.buffer[self.fisica.HeaderLen + nData : ] # descarta o conteúdo já lido
            self.threadResume()
            return(b)
    ...
    def awaitMessage(self):
        while(self.getBufferLen() <= self.fisica.HeaderLen + self.EOPLen):
            time.sleep(0.05)
