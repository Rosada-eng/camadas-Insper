...
def main():
    ...
    duration = (
        5  # s #tempo em segundos que ira aquisitar o sinal acustico captado pelo mic
    )
    ...
    index = peakutils.indexes(y=yf, thres=0.3, min_dist=50, thres_abs=False)
    ...
    combinacoes = []
    for f1 in freq_detectadas:
        for f2 in freq_detectadas:
            combinacoes.append([f1, f2])
    ...
    for combinacao in combinacoes:
        for tecla, frequencias in mySignal.DTMF_matrix.items():
            if set(freq_detectadas) == set(frequencias):
                print(
                    f"Frequências detectadas {set(freq_detectadas)} correspondem à tecla {tecla}"
                )
